...
public class PsychoFormController {
    ...
    @PostMapping("/create")
    public ResponseEntity<?> createForm(@RequestParam Long trainerId,
        ...
        try {
            ...
            List<PsychoFormConvite> convites = service.createConvites(trainerId, req.getSessionId(), frontendBaseUrl);
    ...
    public ResponseEntity<?> validate(@PathVariable String token) {
        try {
            ...
            return ResponseEntity.ok(Map.of(
                    "athleteId", convite.getIdAtleta(),
                    "trainerId", convite.getIdTreinador(),
